...
const Shift1Duties=({setSelectedId, editModal , setEditModal , deleteModal , setDeleteModal , setShift })=>{

    const {buses, status} = useSelector((state) => state.timetable);
    const dispatch = useDispatch()

    const EditBus=async(id)=>{
      const response = await dispatch(fetchASingleBus({id,shiftNumber:1}))
      if(status === 'succeeded'){
        console.log(id)
        setSelectedId(id)
        setEditModal(true)
