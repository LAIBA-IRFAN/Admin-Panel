...
const Shift1Edit = ({selectedId ,editModal , setEditModal , setShift1Table , setVerifyShift1}) => {
  ...
  const EditBus = async()=>{
    try{
      const response = await dispatch(updateASingleBus({id:selectedId,busId:editBus ? busId : singleBus.busId._id
        ,driverId:editDriver ? driverId : singleBus.driverId._id,conductorId:editConductor ? conductorId : singleBus.conductorId._id,shiftNumber:1}))
      console.log(response)
      if(response){
        await dispatch(fetchAllBuses(1))
        setShift1Table(true)
        setEditModal(false)
        setVerifyShift1(true)
      }
    }
    catch(error){
      console.log(error)
